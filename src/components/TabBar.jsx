...
const ICONS={
  ...
  friends:<><circle cx="9" cy="8" r="3.5"/><path d="M2.5 20c0-3.6 2.9-6 6.5-6s6.5 2.4 6.5 6"/><path d="M16 4.6a3.5 3.5 0 0 1 0 6.8M18 14.3c2.1.7 3.5 2.8 3.5 5.7"/></>,
...
export const TABS=[
  ["start","Start"],["ratings","Bewertungen"],["sugg","Vorschläge"],["friends","Freunde"],["cats","Kategorien"],
];
export function TabBar({tab,onTab,badges={},t}){
  return(
    <nav aria-label="Hauptmenü" style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:440,zIndex:100,
      ...
      {TABS.map(([id,label])=>{
        ...
        return(
          <button key={id} onClick={()=>onTab(id)} aria-current={on?"page":undefined} aria-label={badge?label+", "+badge+" neu":undefined}
            ...
            <span style={{position:"relative",display:"flex"}}>
              ...
              {badge>0&&<span aria-hidden="true" style={{position:"absolute",top:-4,right:-8,minWidth:17,height:17,padding:"0 4px",borderRadius:9,background:t.badge,color:t.onBadge,fontSize:10.5,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",border:`2px solid ${t.tabBg}`}}>{badge}</span>}
            </span>
